...
``fetch`` is the daily ingest cycle: sync metadata for tracked
categories. Markdown rendering is deliberately NOT part of it --
rendering paper bodies is heavy (three paced downloads per paper) and
the search/classify path only needs abstracts, so render runs only when
...
def describe_fetch():
    def it_syncs_and_returns_a_summary(data_dir, arxiv):
